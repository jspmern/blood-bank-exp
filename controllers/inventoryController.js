...
let getOrgnazationControllerForDoner = async (req, res, next) => {
    try {
        ...
        res.status(200).send({ message: "Successfully fetched Organization data", success: true, organizations })
    }
    catch (err) {
        console.log(err)
        res.status(500).send({
...
let getOrgnazationControllerForHospital = async (req, res, next) => {
    try {
        ...
        let hospitalId = await inventoryModel.distinct("organization", { hospital })
